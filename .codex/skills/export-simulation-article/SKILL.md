...
```bash
.venv/bin/python .codex/skills/export-simulation-article/scripts/export_article_pdf.py \
...
3. Keep generated source and copied images in the article's `pdf/` folder.
   The exporter normalizes PNG browser captures to RGB JPEGs before Typst compilation to avoid headed-Chrome raster reuse artifacts. Pillow must be installed in the project environment.
4. Verify with `pdfinfo`, `pdftotext`, and rendered pages from `pdftoppm`. Render screenshot-heavy pages in separate `pdftoppm` processes; Poppler can reuse raster state across same-sized consecutive images.
